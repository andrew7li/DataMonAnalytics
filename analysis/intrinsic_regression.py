...
OUTPUT_PATH = BASE_DIR / "analysis" / "sv_intrinsic_regression_output.txt"
...
def clean_for_modeling(df):
    ...
    df = df[df["logMarketPrice"].notna()].copy()
...
def fit_model(df, formula, title):
    ...
    # Add percent-premium interpretation for log coefficients
...
def main():
    ...
    lines.append("SCARLET/VIOLET INTRINSIC CARD VALUE REGRESSION")
    lines.append("=" * 100)
    ...
    # ------------------------------------------------------------
    # Model 2: controls + intrinsic character flags
    # ------------------------------------------------------------
    formula_2 = (
        "logMarketPrice ~ "
        "C(cardRarity) + C(finishType) + C(patternType) + C(setName) + "
    ...
    comparison = pd.DataFrame([
        ...
        {
            "model": "Model 2 Controls + Character Flags",
